...
-- 3. RLS on user_roles: admins can see all, users can see own
...
-- 4. Add admin columns to campaigns and submissions
ALTER TABLE public.campaigns ADD COLUMN IF NOT EXISTS created_by_admin uuid;
ALTER TABLE public.submissions ADD COLUMN IF NOT EXISTS reviewed_by_admin uuid;
ALTER TABLE public.submissions ADD COLUMN IF NOT EXISTS reviewed_at timestamptz;

-- 5. Admin RLS policies for campaigns (CRUD)
...
-- 6. Admin RLS for submissions (update for approve/reject)
...
-- 8. Admin can view all user_roles
CREATE POLICY "Admins can manage roles" ON public.user_roles
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- 9. Storage bucket for campaign images
